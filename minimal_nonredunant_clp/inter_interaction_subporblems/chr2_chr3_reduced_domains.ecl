...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%	Based on the alldifferent\1 global constraint - 
%% 	ensures each variable is pairwise different 
%%	from each other or has a value of zero	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
alldifferent_except(Vars) :-
	% get the length of the input list
	length(Vars, N),

	% loop through all pairs of elements to check 
	...
	( for(I,1,N), param(Vars, N) do
		( for(J, I, N), param(Vars, I) do
			...
			% if it isn't the same element, or an element 
			% with a 0 value
			(I #\= J) ->
...
maximize(RowFile, FreqFile, Non_Zero_Rows) :-
	...
	%% specify the Rows for the inter-interaction submatricies
	...
	% define the Row domains for the inter-interations
	...
	%% define the Freq domains for the inter-interations
	...
	%% apply constraints to the data to ensure they are 
	%% valid 2-tuple pairs based on the 
	%% biological data. note: (0,0) is when we choose 
	%% to not select anything for that
	%% Column row pair

	%Column1051 Constraints
	...
	%% all the Output values will be different - want them to correspond 
	%% to which Row for each has a non-zero value, multiple zeros 
	%% are allowed
